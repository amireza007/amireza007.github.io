...
In order to write constraints in CPP, you need to associate your defined custom types (Quaycranes, AGVs, Containers) with the decision variables of the article. For example, for decision variable $$Z_{l,(m,i),(n,j)}$$ we might need something like `std::vector<tuple<AGV, Container, Container>> z;`