...
- **$$\boldsymbol{\psi _1, \psi _2}$$** QC and ASC op schema (used in **30 and 31** const). These parameters are strictly related to the **stowage plan** of the port (containers on higher tiers are unloaded first while those on lower tiers are loaded first!). These two sets also ensure the sequence of loadings and unloadings into the ship. <br>
  - **Very important note about $$\psi_2(m,i,n,j)$$ and $$\psi_1(m,i,n,j)$$**: ~~The containers $$(m,i)$$ and $$(n,j)$$ should be assigned to the same crane (Hence, for ASCs, they belong to the same block).~~<br>
  Not True according to [this](https://doi.org/10.1016/j.cor.2018.07.005)! Splitting the task of a single bay to multiple QCs may generate a better workload distribution, potentially reducing vessel turnaround time. (Note that for safety measures, two adjacent QCs are not allowed to work on two adjacent bays at the same time. e.g. if QC1 & QC2 is assigned to {B1,B2,B3,B4}&{B5,B6,B7,B8}, B3 could be split between the two, not B4 or B5. QCs should maintain a safety distance)
  - Note that:
    1. For two L container (m,i), (n,j): Tier of *(m,i)* $$\le$$ Tier of *(n,j)*
    2. For two D container (m,i), (n,j): Tier of *(m,i)* $$\ge$$ Tier of *(n,j)*
...
7. As the article considers the block location of the stored unloading containers, we need to have a grasp of **how many ships there are and how many blocks each has (on average)**, which is overlooked in the article!
8. The article does not take into account the **Horizontal movement of QCs**. **S_Q** must be modified then!
9. what is the distribution of *loading* and *unloading* jobs?
10. Sensitivity analysis for `G_Q` snd `G_Y`, since they follow normal distibutions `U(30,60)` 
...
2. $$S_{(m,i)}$$ and $$R_{(m,i)}$$ 
3. How to determine whether assignment of container (m,i) to agv l is compatible with $$\psi_1,\psi_2$$??